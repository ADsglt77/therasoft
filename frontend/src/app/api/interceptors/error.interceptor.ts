...
import { AUTH_COMPONENT_HANDLED_ROUTES, matchesRoute, isApiRequest } from '../../core/constants/api-routes';
import { AuthService } from '../../core/services/auth.service';

/**
 * Interceptor qui gère les erreurs HTTP, notamment les 401 (non autorisé)
 * avec invalidation immédiate de la session Better Auth.
 */
export const errorInterceptor: HttpInterceptorFn = (req, next) => {
  ...
  const forceReLogin = (message?: string) => {
    ...
    notificationService.show('danger', message || 'Session expirée. Veuillez vous reconnecter.', 5000);
